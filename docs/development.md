...
Desktop, the pinned image already present locally and a matching Linux/x64
TypeScript closure in the source repository. Provision it from the committed
lockfile with `bun install --frozen-lockfile --ignore-scripts --os=linux
--cpu=x64`; the closure must include `@typescript/typescript-linux-x64` at the
declared TypeScript version. Tesota performs no install or image pull.
