...
3. **Qualified command isolation.** Complete for the fixed Windows probe. The
   ...
   controls; Codex Windows failed the stricter outside-read and offline controls.
   Network and credential checks include positive canaries, and the descendant
   must prove it started before its later effect can demonstrate settlement. Missing
