...
| Codex-backed Gentle reviewer host | Implemented for one read-only lens | Pi runs a bounded read-only reviewer with Tesota's Codex credential store; a typed `tesota_submit_review` tool now validates the result at the boundary, while Gentle capture and multi-lens settlement remain open |
